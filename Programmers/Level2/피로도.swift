...
//Solution 1
func explore(_ dungeons:[[Int]], _ re: inout Int, _ k: Int, _ cnt: Int) {
    re = max(re, cnt)
    
    for (i,dungeon) in dungeons.enumerated(){
        var n_dungeons = dungeons
        if dungeon[0] <= k && dungeon[1] <= k{
            var n_k = k - dungeon[1]
            n_dungeons.remove(at: i)
            explore(n_dungeons, &re, n_k, cnt+1)
        }
    }
